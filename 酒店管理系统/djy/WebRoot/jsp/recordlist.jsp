...
<body>
    ...
    <span>菜单</span>
			...
			<div class="col-md-12">
					...
					<c:if test="${requestScope.admin!=null}">
					<a href="${pageContext.request.contextPath}/LoginUser?username=admin&password=admin&role=admin"><input value="返回>>" type="button"/></a>
					</c:if>
					<c:if test="${requestScope.admin==null}">
					<a href="${pageContext.request.contextPath}/QueryAllUser"><input value="返回>>" type="button"/></a>
					</c:if>
					...
					<table  style="text-align:center" id="userPager"
						...
						<thead>
							<tr class="bg-primary h4">
								...
								<td><label class=" control-label">登陆IP</label></td>
								<td><label class=" control-label">登录时间</label></td>
								<td><label class=" control-label">用户来源</label></td>
								<td><label class=" control-label">注册时间</label></td>
								<c:if test="${requestScope.admin != null}">
								<td><label class=" control-label">删除</label></td>
								</c:if>
							</tr>
							</thead>
							<!--数据-->
							...
							<tr class="bg-danger" id="${record.username}">
								<td>${record.username}</td>
								<td>${record.finalip}</td>
								<%-- <td style="display: none;">${user.password}</td> --%>
								<td>${record.finalTime}</td>
								<%-- <td>${user.email}</td> --%>
								<td>${record.systemSource}</td>
								<td>${record.registerDate}</td>
								<c:if test="${requestScope.admin != null}">
									<td><a href="/rzy_qiaochuang/DeleteRecord?username=${record.username}&finalTime=${record.finalTime}">×</a></td>
								</c:if>
							</c:forEach>	
						</tbody>
